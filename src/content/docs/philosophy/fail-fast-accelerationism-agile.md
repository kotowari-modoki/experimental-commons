...
## シリコンバレーの速度文化はどこに入るのか

Lean Startup やシリコンバレーのスタートアップ文化が重なると、fail-fast と加速度主義はさらに似て見えます。これらには「速度」「実験」「既存の遅さへの不信」があるからです。

Paul Graham は 2012 年の `Startup = Growth` で、スタートアップを「速く成長するよう設計された会社」と説明しています。この文脈では、速さは思想上の加速というより、学習速度と成長率を上げるための経営上の圧力です。
...
したがって、Lean Startup や Silicon Valley culture は、fail-fast と加速度主義の中間に見える層です。Lean Startup の fail fast は、基本的には **仮説検証を速くして損失を限定する** ための実務です。Facebook 的な `move fast` も、**市場機会を逃さず、プロダクト学習を速める** ための企業文化です。資本主義や技術社会そのものを加速させることで別の秩序や崩壊を問う加速度主義とは、対象も射程も違います。
...
ただし、間に Lean Startup や Silicon Valley の速度文化を置くと、似て見える理由は説明しやすくなります。問題を長く隠すより、早く露出させる。変化を避けるより、変化の中で学ぶ。既存の均衡に安住するより、動きを強める。この感覚は三者に部分的に共通しています。

しかし、同じ「速くする」でも、何を速くするのかが違います。

| 項目 | fail-fast | Lean Startup / Silicon Valley culture | 加速度主義 |
| --- | --- | --- | --- |
| 主な領域 | ソフトウェア工学、信頼性設計、開発プロセス | 事業開発、スタートアップ経営、プロダクト文化 | 哲学、政治思想、批判理論、技術社会論 |
| 失敗・加速の対象 | バグ、障害、不正状態 | 事業仮説、プロダクト、成長、市場機会 | 資本主義、技術、抽象化、社会変容 |
| 目的 | 早期検知、影響範囲の縮小、修正 | 市場学習、成長率、機会損失の回避 | 既存秩序の変容、脱出、または崩壊をめぐる理論 |
| 制御の前提 | 小さく試し、観測し、戻れるようにする | 実験と成長を速めるが、巻き込む範囲は広がる | 制御可能性そのものが争点になりやすい |
| 実務上の態度 | リスクを小さくして学ぶ | リスクを取り、市場から速く学ぶ | 大きな構造変化をどう扱うかを問う |

この表の真ん中を置くと、混同の理由と、分けるべき理由が見えやすくなります。fail-fast は工学的な早期検知、Lean Startup / Silicon Valley culture は市場学習と成長の高速化、加速度主義は社会構造の加速をめぐる思想です。
...
Lean Startup やシリコンバレーの速度文化を扱うときは、さらに一段注意が必要です。仮説検証や成長のための速度は有効ですが、実験対象が市場やユーザーに広がるほど、失敗のコストも外へ広がります。

一方で、加速度主義を読むときは、単なる「スピード礼賛」として読まない方がよいです。加速度主義は、技術と資本主義の速度をめぐる思想であり、そこには解放、支配、崩壊、制御不能性が入り混じっています。

これらを混ぜると、たとえば「速くやればよい」「失敗が多いほどよい」「ブレーキは悪い」という粗い議論になりやすい。これは、fail-fast の実務にも、Lean Startup の運用にも、加速度主義の理解にもよくありません。

むしろ、次の対比で覚えるとよさそうです。

| 観点 | fail-fast | Lean Startup / Silicon Valley culture | 加速度主義 |
| --- | --- | --- | --- |
| 一言で言うと | 制御された早期失敗 | 市場学習と成長のための高速化 | 構造的な加速の思想 |
| よい使い方 | 小さな実験と早い検知 | 仮説検証、顧客学習、機会損失の回避 | 技術・資本・社会変容の読解 |
| 悪い使い方 | 雑に壊して学習と言う | ユーザーや社会に失敗コストを押し出す | 速さそのものを善とみなす |
| 実務上の警句 | 早く失敗するほど、戻れる設計が必要 | 速く学ぶほど、誰を実験に巻き込むかを問う必要がある | 加速するほど、誰が代償を払うのかを問う必要がある |
