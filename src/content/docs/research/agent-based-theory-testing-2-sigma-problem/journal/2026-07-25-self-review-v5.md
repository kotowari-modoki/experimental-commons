---
title: v5を30分査読し、個別指導の高得点を「必要なルールが記憶に残ったか」から説明し直す
description: 学習者の異質性を初めて導入したv5を、実験データベースと学習後の記憶データへ戻って査読した。主数値は維持しつつ、個別指導の高得点を誤概念修正へ帰属した説明を、必要なルールが記憶に残っていたことへ訂正した。
date: 2026-07-25
status: seed
tags:
  - research
...
<!--
ABOUTME: 学習者の異質性を初めて導入したv5を、実験DBと記憶データへ戻って査読した記録。
ABOUTME: 維持された主数値と、必要なルールが記憶に残ったかへ訂正した説明を分ける。
-->
...
- **個別指導（1on1）**: 教師役（Tutor）が学習者役を一人ずつ教える条件
- **実験実行（run）**: 一組の設定で実験を最初から最後まで動かした1回分。DBはその結果を保存する実験データベース
- **学習者プロファイル**: 能力、興味、誤概念などをまとめた人物設定。highとlowは高能力・低能力として用意した設定を指す
- **記憶データ（memory）**: 授業後に学習者役へ残した要約。テストでは、会話全文ではなく、この記憶データを使って回答する
- **ルールカバレッジ**: 問題を解くために必要な規則が、記憶データへどこまで残っているか。Blue=5、Green=2、Yellow=7は、問題で使う色ごとの基底点である
- **n**: 比較に含まれる人数。n=4は4名を意味する
- **L1、L4、L6**: 8問ある評価課題の番号。本文のrecallは規則の想起、debuggingは誤りの修正、inductionは例から規則を見つける課題を指す
