...
[実行データを公開対象へ戻した](/experimental-commons/research/agent-based-theory-testing-2-sigma-problem/journal/2026-07-14-include-run-data-before-self-review/)あと、私はセルフ査読を始めた。最初の対象は、分析ドキュメントを持たないパイロット世代のv1からv3である。

査読には約80分かかった。実行runを対応づけ、論文や開発ログの主張をデータとコードへ戻って確かめる作業は、思った以上に集中力を求められた。結構大変だった一方で、当時の実装が何を測っていたのかを理解し直す勉強にもなった。

## この記事で使う用語

- **集団授業（classroom）**と**1対1指導（1on1）**: 比較した二つの教育条件。
- **教育なし条件（`no_education`）**: 教育セッションを受けずに評価問題を解く比較条件。
- **実行単位（run）とデータベース（DB）**: runは一まとまりの実験実行、DBは各runの条件、応答、得点などを保存する場所。
- **プロンプト（prompt）**: LLMへ渡す指示文。DBに保存された文面と、実行時に実際に渡した文面が同じとは限らないため、コードと設定も照合した。
- **学習メモリ**: 教育後に学習者役が作り、評価問題で参照する要約。
- **所見の扱い**: `fixed`は修正済み、`accepted-risk`は限界を明記して受け入れる、`wont-fix`は理由を記録して今回は直さない、という査読上の分類。
