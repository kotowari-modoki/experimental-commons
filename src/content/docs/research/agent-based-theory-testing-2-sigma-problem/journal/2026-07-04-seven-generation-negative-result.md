...
> **Update（2026-07-19）:** v1–v3のセルフ査読により、v1の天井原因とv3の`no_education`の数値に訂正が必要だと分かりました。このページには当時の理解を残し、訂正内容は[v1–v3を80分かけて査読し、天井効果の説明を直す](/experimental-commons/research/agent-based-theory-testing-2-sigma-problem/journal/2026-07-19-self-review-v1-v3/)に記録しています。

## この記事で使う用語

- **集団授業（Classroom）**: 教師役の説明や討論を複数の学習者役が共有する条件。
- **1対1指導（1on1 / tutoring）**: 教師役と学習者役が個別にやり取りする条件。正解するまで教える**完全習得型の個別指導（mastery tutoring）**とは区別する。
- **実行単位（run）とデータベース（DB）**: runは一まとまりの実験実行、DBは各runの条件、応答、得点などを保存する場所。
- **学習メモリ（memory）**: 教育セッション後に学習者役が作り、評価問題を解くときに参照する短い要約。この実験で「学習した内容」に相当する。
- **プロンプト（prompt）**: LLMへ渡す指示文。評価問題へ答えが漏れると、学習効果ではなく指示文を読めた効果を測ってしまう。
- **`n`（比較に含めた人数・試行数）**: 箇所によって学習者数またはタイプごとの試行数を指すため、その都度、何を数えた値かを併記する。
- **交絡**: 比べたい教育条件以外の違いも結果を動かし、原因を一つに決められない状態。**疑似反復**は、同じ授業や教師を共有した観察を独立した人数のように数える問題。
- **床効果・天井効果**: 得点が最低値または最高値へ張り付き、条件差を見分けにくくなる状態。
