---
title: v4を25分で査読し、集団授業の高得点をBloom理論の反証と読まない
description: 最初の本番実験で得た集団授業47%、1対1指導28%、教育なし0%をデータベースと生データへ戻って確認した。主数値は維持しつつ、低得点タスクの説明を訂正し、4往復で終わる1対1指導を完全習得型の個別指導と同一視できない限界を記録する。
date: 2026-07-20
status: seed
tags:
  - research
...
- **集団授業（Classroom）**: 一つの授業や討論を複数の学習者役が共有する条件。
- **1対1指導（1on1 / Tutor）**: 教師役と学習者役が個別に4往復する条件。正解するまで教え続ける**完全習得型の個別指導（mastery tutoring）**ではない。
- **実行単位（run）とデータベース（DB）**: runは一まとまりの実験実行、DBは条件、応答、得点などを保存する場所。
- **学習メモリ（memory）**: 教育後に学習者役が作り、評価問題を解く際に参照する要約。
- **採点器（scorer）**: 応答を正誤判定する処理。**完全一致採点（exact-match scorer）**では、意味が合っていても指定表記と一致しなければ誤答になることがある。
- **疑似反復**: 同じ授業や討論を共有した複数人を、完全に独立した観察のように数えてしまう問題。
