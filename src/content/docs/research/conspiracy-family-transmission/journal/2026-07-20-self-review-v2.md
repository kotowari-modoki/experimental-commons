---
title: v2を40分で査読し、条件を伏せた採点120件がすべて既定値だったと確認する
description: 初のLLM版を主張、コード、生データへ戻って査読した。主指標はレポートと一致した一方、条件を伏せた採点120件はすべて既定値へ退化しており、会話品質を測れていなかった。
date: 2026-07-20
status: seed
tags:
  - research
...
ABOUTME: 維持された主指標と、全件が既定値へ退化した条件を伏せた採点器を分けて扱う。
-->

# v2を40分で査読し、条件を伏せた採点120件がすべて既定値だったと確認する
...
- **家族役のLLMエージェント**: 大規模言語モデルに家族内の役割を与え、会話と信念状態を生成させる実験上の登場人物。
- **採点器（scorer）**: 会話品質を評価する別の処理。今回は介入条件を伏せて採点する設計だった。
- **既定値（fallback）**: 採点処理が有効な結果を返せなかったときに代わりに保存する値。120件すべてが同じ既定値だったため、実際の会話品質を測れていない。
- **論文草稿（paper draft）とレポート**: 前者は研究全体を論じる原稿、後者は各実験の結果を生成コードからまとめた文書。
- **`n`**: 比較に含めた家族数。この実験の各条件は2家族（n=2）だった。
- **仮説判定の内部名**: `inconclusive`は判定不能、`not supported`は設定した支持条件を満たさなかった状態。
