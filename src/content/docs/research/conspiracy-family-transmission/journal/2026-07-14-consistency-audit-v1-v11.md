---
title: v1–v11を遡って不整合を点検する
description: 実験を一段ずつ進める中でドキュメントと実際の進行がずれるため、v1からv11まで遡って整合性を点検し、最低値に張り付いた主指標などに由来する不整合5件と、生成物を元から作り直す修正方針を記録する。
date: 2026-07-14
status: seed
tags:
  - research
...
- **介入条件の内部名**: `direct_correction`は陰謀的説明を直接訂正する条件、`check_in`は後日様子を尋ねる条件です。
- **`family_avoidance_intent`（家族を避ける意図）**: 家族との接触を避けたい度合いを記録する予定だった結果指標です。
- **床（floor）**: 値が下限に張り付いて条件間の差が出ない状態。差が小さいのではなく、そもそも検出対象が下限に達しています。
- **MDE（最小検出可能効果）**: 実験設計上、見つけられると見込んだ最小の差。本来はサンプル数 `n`（ここでは家族・試行の数）を増やせば小さくできます。しかし `loop_strength` は床に達していてノイズが問題なのではないため、nで縮めるという説明（§17）が状況に合っていませんでした。
- **評価基準表による採点（rubric scoring）と条件を伏せた評価者（blind judge）**: 会話を評価する方式。v12では前者を使っておらず、後者も途中で採用をやめています。§17の記述はその名残でした。
