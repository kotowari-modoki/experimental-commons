---
title: v1をセルフ査読し、生成コードとレポートの食い違いを直す
description: あらかじめ決めた規則で動く初代パイロットを、主張・コード・生データへ戻って査読した。二つの仮説判定が再生成で監査前へ戻る問題を直し、指標名と定性例も修正した記録。
date: 2026-07-20
status: seed
tags:
  - research
...
- **決定論モデル**: 同じ条件なら同じ結果になる、あらかじめ定めた更新規則で家族の状態を変えるモデル。v1ではLLMに自由な会話を生成させていない。
- **レポート**: 実験結果を生成コードからまとめた文書。本文だけを手で直すと、再生成時に修正が消える。
- **仮説判定の内部名**: `assumed`は設計上の仮定、`partially supported`は一部だけ支持、`supported`は設定した判定条件を満たした状態。
- **確信レンジ（Confidence Range）**: 家族内で最も高い確信度から最も低い確信度を引いた値。統計的な分散ではない。
- **条件を伏せた採点（blind scoring）**: どの介入条件の会話かを採点側へ知らせずに評価する方法。
