...
v3は、大規模言語モデル（LLM）に8種類の家族役を演じさせる実験だった。

- 制度への不信が強い祖父母役（`grandparent`）
- 不安を感じやすい養育者役（`parent_a`）
- 制度を信頼する養育者役（`parent_b`）
- オンラインで情報を調べる叔父・叔母役（`uncle_or_aunt`）
- 対立を避けるいとこ役（`cousin`）
- 家族関係の調和を優先する子ども・若者役（`child_or_young_adult`）
- 家族の対話を仲介する橋渡し役（`bridge_person`）
- 陰謀的説明を強く信じる親族役（`convinced_relative`）

比較した7条件は、無介入、直接否定、証拠の確かめ方を支援する「認識的支援」、事前警告、橋渡し役による仲介、匿名情報源、橋渡し役なしである。本文では、実装上の条件名である`anonymous_source`（匿名情報源）や`no_bridge_person`（橋渡し役なし）も括弧内に残す。

登場人物が自己申告する信念状態（`belief_state`）には、`uncertain`（不確か）、`mixed`（中間状態）、`conspiracy_account`（陰謀的説明を採用）、`official_account`（公式説明を採用）の4判定がある。DVは「結果指標」のことで、条件を変えたときに何が動いたかを見る測定値を指す。`confidence`は確信度指標、`intent`は検証や拡散などの意図を表す指標である。

reportは実験結果レポート、paper draftは論文草稿を指す。盲検scorerは、実験条件を伏せて会話品質を評価する別の判定器である。fallbackは、その判定器が正常な評価を返せなかったときに入る既定値を意味する。
...
5. 全発話の役柄外テキストと、盲検の会話品質評価器（scorer）の値を確認する
6. 分類ラベルから作る結果指標（DV）と、確信度などの連続指標を分ける
...
今回、判定カテゴリの境界に依存する三つの結果指標（DV）を使用しないと決めた。
...
一方、確信度（`confidence`）、各種の意図（`intent`）、家族の緊張（`family tension`）、信念のばらつき（`belief polarization`）のような連続指標は、ラベル境界を必要としない。値に分散があるだけで妥当性が証明されるわけではないが、差分や軌跡としては引き続き検討できる。
...
条件平均の小さな差を見つけたら、何体のエージェントが動いた結果なのかまで降りて確認する必要がある。固定エージェントが分類型の結果指標を埋めている場合、平均値だけでは飽和が見えない。
...
| 仮説 | 査読前 | 査読後 | 理由 |
| --- | --- | --- | --- |
| H1 家族情報源の増幅 | supported | **撤回・判定不能** | ラベル由来の結果指標で、差は祖父母1体に依存 |
| H2 直接否定による防御 | supported | **撤回・判定不能** | ラベル遷移由来で、1/8の単一遷移に依存 |
| H3 認識的支援 | supported | **条件付きで残る** | 検証意図0.691 > 0.613。連続指標だがn=2 |
...
## 盲検の会話品質評価器は204件すべて既定値だった

v2に続き、v3の盲検会話品質評価器（scorer）も退化していた。204件すべてが、品質0.5、認識的品質`neutral`（中立）、支配的行動`question`（質問）という同一の既定値（fallback）だった。

盲検スコアは仮説判定や主要指標には使われていないため、今回の撤回とは独立した問題である。ただし、v3のreportとpaper draftには退化が開示されていなかった。既定値で行が埋まっているだけの状態を、測定成功とは扱わない。

自己申告ツールの連続指標は9〜15種の値を取り、分散を持って動いていた。一方、盲検の会話品質評価器は完全に退化していた。「判定機が動いた」と一括りにはできない。

## 発話の19.3%に役柄外の前置きが入っていた

326発話中63件、19.3%に、演者モデルが実験環境のスキル指示へ反応した役柄外の前置きが入っていた。介入内容を家族へ伝える進行役（moderator）の11発話も含まれるため、比較したい介入そのものを送る会話文脈が汚染されている。

前置きの多くは会話本文の前に置かれていたが、形式は一つではなく、単純なルールでは安全に除去できない。生データは事後整形せず、汚染を論文の制約事項（Limitations）へ開示する。本質的な解決は、生成側で検出し、清浄な環境で再実行することである。
...
7. 人間の盲検判断前に、エージェントが会話品質評価器の出力を見せない
...
- 盲検の会話品質評価器が全件既定値だったことと、役柄外前置きの混入を制約事項へ開示する
